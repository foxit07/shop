...
@section('content')
    ...
    <div class="tab-content" id="provider">
        <div class="tab-pane fade show active" id="general" role="tabpanel" aria-labelledby="general-tab">
            <div class="mt-3">
                <form action="{{ route('providers.store') }}" method="POST" id="providerForm">
                    {{ csrf_field() }}
                    <div class="form-group">
                        <label for="provider">Provider</label>
                        <input type="text" class="form-control" id="provider" placeholder="Provider" name="name">
                    </div>
                </form>
            </div>
        </div>
