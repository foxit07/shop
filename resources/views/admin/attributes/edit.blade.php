...
@section('content')
    ...
    <div class="tab-content" id="attributeContent">
        <div class="tab-pane fade show active" id="general" role="tabpanel" aria-labelledby="general-tab">
            <div class="mt-3">
                <form action="{{ route('attributes.update', $attribute->id) }}" method="POST" id="attributesForm">
                    {{ csrf_field() }}
                    {{ method_field('PATCH') }}
                    <div class="form-group">
                        ...
                        <select class="form-control" id="group" name="group_id">
                            @forelse($groups as $group)
                                <option value="{{ $group->id }}" {{ $group->id == $attribute->group_id ?  'selected' : '' }}> {{ $group->name }} </option>
                            @empty
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="attribute" name="name"  value="{{ $attribute->name }}">
                    </div>
                </form>
            </div>
        </div>
